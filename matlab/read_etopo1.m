function [h,x,y,ii,jj] = read_etopo1(xi,yi);
% [h,x,y] = read_etopo1(xi,yi);
%
% Read the 1 minute ETOPO1 dataset to a regular grid that encompasses the
% end values of (XI,YI)
...
nc=netcdf([DatasetDir '/ETOPO1_Bed_g_gmt4.grd'],'read');

Xe0=nc{'x'}(1); Xe1=nc{'x'}(end); Xe=nc{'x'}(:);
...
h=nc{'z'}(jj,i1)'; % (i,j) indexing
%h=nc{'bath'}(jj,i1); % (j,i) indexing
if ~isempty(i2)
 h(length(i1)+(1:length(i2)),:)=nc{'z'}(jj,i2)'; % (i,j) indexing
