function [H,x,y] = read_sandwell_image(xi,yi)
% [H,x,y] = read_sandwell_image(xi,yi)

reg=[min(yi(:)) max(yi(:)) min(xi(:)) max(xi(:))];
[H,y,x]=mygrid_sand(reg,1);
H=H';
...
DatabasesDir = '/Users/cdh/NOBACKUP';
DatabasesDir = '/net2/aja/datasets';
...
for i = 1:length(wlon);
	...
	fid = fopen([DatabasesDir '/sandwell_topo_14.1.img'], 'r','b');
