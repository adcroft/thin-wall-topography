...
def main():
  ...
  parser.add_argument('--variable', type=str,
                      nargs='?', default='depth',
                      help='Name of height variable that is negative in ocean.')

  optCmdLineArgs = parser.parse_args()

  createGUI(optCmdLineArgs.edits_file, optCmdLineArgs.topography_file, optCmdLineArgs.variable)
...
def createGUI(edits_file, topo_file, var='depth'):
  ...
  for n,(i,j,z) in enumerate(zip(iEdit, jEdit, zEdit)):
    ...
    depth[j,i] = -z #1/2 degree edits are positive so they are for depth
